...
class DocumentController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if($validation->fails()) {            
            ...
            $table->openai_system_content = $request['openai_system_content'] ?? '';
            $table->openai_user_content = $request['openai_user_content'] ?? '';
            $table->description         = $request['description'] ?? '';
            $table->template            = $request['template'] ?? '';            
            $table->image               = $image;           
            $table->sort_order          = ( $request['sort_order'] == '' ) ? 0 : $request['sort_order'];
            $table->status              = ( $request['status'] == '' ) ? 0 : $request['status'];
