...
class CustomerAddressController extends Controller
{
    ...
    {
        //=== check permision
        if(!has_permision(['customers'])){ return redirect( admin_url('dashboard') ); }
    ...
    {
        //=== check permision
        if(!has_permision(['customers'=>'RW'])){ return redirect( admin_url('dashboard') ); }
        ...
        $countries = AllFunction::countries([]);       
    ...
    public function store(Request $request)
    {
        ...
        if($apply_action){
            ...
            if( $apply_action == 'active' && $id_array){      
                ...
                return redirect( admin_url('customers') )->with('message','Selected item Updated successfully');
            }
                ...
                if($result){
                    foreach($result as $val){
                        $delArr = array(
                            'file_path'=>'public/uploads/customers',
                ...
                return redirect( admin_url('customers') )->with('message','Selected item deleted successfully');
            }  
        }
        ...
        if($validation->fails()) {            
            ...
            $table = Customers_address::where('customer_id', $customer_id)->get()->first();   
            if($table){  
                $table->name         = $request['name'];            
                ...
                $table->zone_id      = $request['zone_id'];       
                $table->city         = $request['city'];       
                $table->default      = 1; 
                $table->save();           
            }
            else{
                $table = new Customers_address;
            ...
            return redirect( admin_url('customers') )->with('message','Customer created successfully');
        }        
    }
    ...
    public function show(string $id)
    {
        ...
        ->leftJoin('countries', 'countries.country_id', '=', 'customers_address.country_id')    
        ->leftJoin('zones', 'zones.zone_id', '=', 'customers_address.zone_id')         
        ->select('customers.*', 'customers_address.*','countries.country_name','zones.zone_name')
    ...
    public function update(Request $request, string $id)
    {
        ...
        if($validation->fails()) {            
            ...
            $table = Customers_address::where('customer_id', $id)->get()->first();           
            if($table){                  
                $table->name         = $request['name'];            
                $table->company      = $request['company'];
                $table->address      = $request['address'];
            ...
            return redirect( admin_url('customers') )->with('message','Customer updated successfully');
        }
    }
