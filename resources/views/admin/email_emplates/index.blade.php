...
@section('page-content')
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="row my-3">
                            ...
                            <form id="filterForm" name="filterForm" method="get" action="{{ route('email-templates.index') }}"> 
                            <div class="row">
                            ...
                            @if(has_permision(['emailtemplates'=>'RW']))
                            <div class="col-lg-2 col-md-12 col-12">
                                <div class="text-end">
                                <a href="{{ route('email-templates.create') }}" class="btn btn-secondary">+ Add New</a>
                                </div>
                            </div> 
                            @endif                        
                        </div>                       
                        
                        <form id="applyForm" name="applyForm" method="post" action="{{ route('email-templates.index') }}" >  
                        @csrf
                        ...
                        <table class="table table-hover table-striped">                            
                            ...
                            <tbody>
                                @if($results)
                                    ...
                                    <tr id="row-{{ $val['email_template_id'] }}">
                                        <td><input class="form-check-input selected-chk" type="checkbox" name="id[]" value="{{ $val['email_template_id'] }}"></td>
                                        <td>{{ $start_count }}</td>
                                        <td>{{ $val['title'] }}</td>                                    
                                        <td>
                                            @if($val['status'] == '1')                                                
                                        ...
                                        <td class="text-end">
                                            <a href="{{ route('email-templates.show',$val['email_template_id']) }}" class="btn btn-md" title="View"><i class="bi bi-columns-gap text-secondary"></i></a>

                                            @if(has_permision(['emailtemplates'=>'RW']))
                                            <a href="{{ route('email-templates.edit',$val['email_template_id']) }}" class="btn btn-md" title="Edit"><i class="bi bi-pencil-square text-success"></i></a>
                                            <button type="button" class="btn btn-md delete"                                             
                                            onclick="delete_row({{ $val['email_template_id'] }})"                         
    ...
    const delete_row = (id) =>{ 
        ...
        swal({		  
            ...
            var url = "{{ route('email-templates.destroy','id') }}"
