...
@section('page-content')
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="row my-3">
                            ...
                            <form id="filterForm" name="filterForm" method="get" action="{{ route('banner-category.index') }}"> 
                            <div class="row">
                            ...
                            @if(has_permision(['banners'=>'RW']))
                            <div class="col-lg-2 col-md-12 col-12">
                                <div class="text-end">
                                <a href="{{ route('banner-category.create') }}" class="btn btn-secondary">+ Add New</a>
                                </div>
                            </div>  
                            @endif                       
                        </div>                       
                        
                        <form id="applyForm" name="applyForm" method="post" action="{{ route('banner-category.index') }}" >  
                        @csrf
                        ...
                        <table class="table table-hover table-striped">                            
                            ...
                            <tbody>
                                @if($results)
                                    ...
                                    <tr id="row-{{ $val['bannercat_id'] }}">
                                        <td><input class="form-check-input selected-chk" type="checkbox" name="id[]" value="{{ $val['bannercat_id'] }}"></td>
                                        <td>{{ $start_count }}</td>
                                        <td><a href="{{ route('banner-category.banners.index',$val['bannercat_id']) }}">{{ $val['name'] }} ({{ count($val['banners']) }})</a></td>   
                                        <td>{{ $val['width'] }}</td>  
                                        <td>{{ $val['height'] }}</td>                                            
                                        <td>
                                            @if($val['status'] == '1')                                                
                                        ...
                                        <td class="text-end">
                                            <a href="{{ route('banner-category.show',$val['bannercat_id']) }}" class="btn btn-md" title="View"><i class="bi bi-columns-gap text-secondary"></i></a>

                                            @if(has_permision(['banners'=>'RW']))
                                            <a href="{{ route('banner-category.edit',$val['bannercat_id']) }}" class="btn btn-md" title="Edit"><i class="bi bi-pencil-square text-success"></i></a>
                                            <button type="button" class="btn btn-md delete"                                             
                                            onclick="delete_row({{ $val['bannercat_id'] }})"                                  
    ...
    const delete_row = (id) =>{
        ...
        swal({		  
            ...
            var url = "{{ route('banner-category.destroy','id') }}"
