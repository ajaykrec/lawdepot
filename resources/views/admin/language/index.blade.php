                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="row my-3">
                            ...
                            <form id="filterForm" name="filterForm" method="get" action="{{ route('language.index') }}"> 
                            <div class="row">
                            ...
                            @if(has_permision(['language'=>'RW']))
                            <div class="col-lg-2 col-md-12 col-12">
                                <div class="text-end">
                                <a href="{{ route('language.create') }}" class="btn btn-secondary">+ Add New</a>
                                </div>
                            </div>    
                            @endif                     
                        </div>                       
                        
                        <form id="applyForm" name="applyForm" method="post" action="{{ route('language.index') }}" >  
                        @csrf
                        ...
                        <table class="table table-hover table-striped">                            
                            ...
                            <tbody>
                                @if($results)
                                    ...
                                    <tr id="row-{{ $val['language_id'] }}">
                                        <td>
                                            @if($val['default'] != 1)
                                            <input class="form-check-input selected-chk" type="checkbox" name="id[]" value="{{ $val['language_id'] }}">
                                            @endif
                                        </td>
                                        <td>
                                        @if($val['image'])  
                                            <img src="{{ url('/storage/uploads/language/'.$val['image']) }}" class="img-thumb" height="25">  
                                        @else
                                            <img src="{{ url('/admin-assets/assets/img/no-photos.png') }}" class="img-thumb" height="25">
                                        @endif      
                                        </td>
                                        <td>{{ $val['name'] }}</td>   
                                        <td>{{ $val['code'] }}</td>                                     
                                        ...
                                        <td class="text-end">
                                            @if(has_permision(['language'=>'RW']))
                                            <a href="{{ route('language.edit',$val['language_id']) }}" class="btn btn-md" title="Edit"><i class="bi bi-pencil-square text-success"></i></a>

                                                @if($val['default'] != 1)
                                                <button type="button" class="btn btn-md delete"                                             
                                                onclick="delete_row({{ $val['language_id'] }})"         
