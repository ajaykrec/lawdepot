                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="row my-3">
                            ...
                            <form id="filterForm" name="filterForm" method="get" action="{{ route('testimonial.index') }}"> 
                            <div class="row">
                                ...
                                <div class="col-lg-4 col-md-6 col-12">
                                <div class="mb-2">
                                <input type="text" class="form-control" id="designation" name="designation" value="{{ $designation ?? '' }}" placeholder="Designation">                                
                                </div>  
                                </div> 
                            ...
                            @if(has_permision(['testimonial'=>'RW']))
                            <div class="col-lg-2 col-md-12 col-12">
                                <div class="text-end">
                                <a href="{{ route('testimonial.create') }}" class="btn btn-secondary">+ Add New</a>
                                </div>
                            </div>   
                            @endif                      
                        </div>                       
                        
                        <form id="applyForm" name="applyForm" method="post" action="{{ route('testimonial.index') }}" >  
                        @csrf
                        ...
                        <table class="table table-hover table-striped">                            
                            ...
                            <tbody>
                                @if($results)
                                    ...
                                    <tr id="row-{{ $val['testimonial_id'] }}">
                                        <td><input class="form-check-input selected-chk" type="checkbox" name="id[]" value="{{ $val['testimonial_id'] }}"></td>
                                        <td>{{ $start_count }}</td>
                                        <td>{{ $val['name'] }}</td>   
                                        ...
                                        <td class="text-end">
                                            <a href="{{ route('testimonial.show',$val['testimonial_id']) }}" class="btn btn-md" title="View"><i class="bi bi-columns-gap text-secondary"></i></a>

                                            @if(has_permision(['testimonial'=>'RW']))
                                            <a href="{{ route('testimonial.edit',$val['testimonial_id']) }}" class="btn btn-md" title="Edit"><i class="bi bi-pencil-square text-success"></i></a>
                                            <button type="button" class="btn btn-md delete"                                            
                                            onclick="delete_row({{ $val['testimonial_id'] }})"                      
