                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="row my-3">
                            ...
                            <form id="filterForm" name="filterForm" method="get" action="{{ route('pages.index') }}"> 
                            <div class="row">
                            ...
                            @if(has_permision(['pages'=>'RW']))
                            <div class="col-lg-2 col-md-12 col-12">
                                <div class="text-end">
                                <a href="{{ route('pages.create') }}" class="btn btn-secondary">+ Add New</a>
                                </div>
                            </div>   
                            @endif                      
                        </div>                       
                        
                        <form id="applyForm" name="applyForm" method="post" action="{{ route('pages.index') }}">  
                        @csrf
                        ...
                        <table class="table table-hover table-striped">                            
                            ...
                            <tbody>
                                @if($results)
                                    ...
                                    <tr id="row-{{ $val['page_id'] }}">
                                        <td><input class="form-check-input selected-chk" type="checkbox" name="id[]" value="{{ $val['page_id'] }}"></td>
                                        <td>{{ $start_count }}</td>
                                        <td>{{ $val['name'] }}</td>   
                                        <td>{{ $val['slug'] }}</td>                                     
                                        <td>
                                        ...
                                        <td class="text-end">
                                            ...
                                            @if(has_permision(['pages'=>'RW']))
                                            <a href="{{ route('pages.edit',$val['page_id']) }}" class="btn btn-md" title="Edit"><i class="bi bi-pencil-square text-success"></i></a>
                                            <button type="button" class="btn btn-md delete" 
                                            onclick="delete_row({{ $val['page_id'] }})"                             
