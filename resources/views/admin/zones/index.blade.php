                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="row my-3">
                            ...
                            <form id="filterForm" name="filterForm" method="get" action="{{ route('zones.index') }}"> 
                            <div class="row">

                                <div class="col-lg-4 col-md-6 col-12">
                                <div class="mb-2">
                                <input type="text" class="form-control" id="zone_name" name="zone_name" value="{{ $zone_name ?? '' }}" placeholder="Zone Name">                                
                                </div>  
                                </div>  

                                <div class="col-lg-4 col-md-6 col-12">
                                <div class="mb-2">
                                <input type="text" class="form-control" id="zone_code" name="zone_code" value="{{ $zone_code ?? '' }}" placeholder="Zone Code">                                
                                </div>  
                                </div> 
                            ...
                            @if(has_permision(['zones'=>'RW']))
                            <div class="col-lg-2 col-md-12 col-12">
                                <div class="text-end">
                                <a href="{{ route('zones.create') }}" class="btn btn-secondary">+ Add New</a>
                                </div>
                            </div>    
                            @endif                     
                        </div>                       
                        
                        <form id="applyForm" name="applyForm" method="post" action="{{ route('zones.index') }}" >  
                        @csrf
                        ...
                        <table class="table table-hover table-striped">                            
                            ...
                            <tbody>
                                @if($results)
                                    ...
                                    <tr id="row-{{ $val['zone_id'] }}">
                                        <td>                                            
                                            <input class="form-check-input selected-chk" type="checkbox" name="id[]" value="{{ $val['zone_id'] }}">                                           
                                        </td>                                        
                                        <td>{{ $val['zone_name'] }}</td>   
                                        <td>{{ $val['zone_code'] }}</td>      
                                        ...
                                        <td class="text-end">
                                            @if(has_permision(['zones'=>'RW']))
                                            <a href="{{ route('zones.edit',$val['zone_id']) }}" class="btn btn-md" title="Edit"><i class="bi bi-pencil-square text-success"></i></a>
                                                
                                            <button type="button" class="btn btn-md delete"                                             
                                            onclick="delete_row({{ $val['zone_id'] }})"         
